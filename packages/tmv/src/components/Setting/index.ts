...
const Setting = {
    popoverContent() {
        const content = createDomWithClass('tmv')
    ...
    render() {
        const setting = createDomWithClass('tmv-setting')
        const settingBtn = createDomWithClass('tmv-setting-btn')
        ...
        setting.appendChild(popover.render(settingBtn, this.popoverContent()))

        return SvgBtn.render(setting)
    }
}
